...
export default function PrivateTeamProjectPage() {
  return (
    <main className="flex min-h-screen flex-col px-8 py-24 gap-4 sm:px-24">
      ...
      <SecondaryTitle customClassName="mt-8 text-2xl sm:text-3xl md:text-4xl">
        개요
      </SecondaryTitle>
      <UnorderedBulletList>
        <li>
      ...
      <SecondaryTitle customClassName="mt-8 text-2xl sm:text-3xl md:text-4xl mb-1">
        React Native 프로젝트를 Expo로 만든 이유?
      </SecondaryTitle>
      <Paragraph>
        해당 앱은 iOS + Android를 타겟으로 하는 모바일 앱이지만, MacOS 뿐만
        아니라 Windows로도 개발하기 때문에, Expo를 사용하는 것이 유일한
        선택지입니다. 실제 React Native 공식 문서에서도{' '}
        ...
        타겟인 이 앱을 MacOS뿐만 아니라 Windows로도 개발할 수 있게 만들었습니다.
      </Paragraph>
      ...
      <Paragraph customClassName="mb-0">
        빌드 구축 전에는 Expo Go를 활용하였습니다. Expo Go는 개발 중인 앱을 따로
        빌드하여 설치하지 않고도 QR 코드를 스캔하거나 <Tag>exp://</Tag>로
        시작하는 URL을 클릭하여 앱의 프리뷰를 볼 수 있게 해주는 앱입니다. 이
        앱을 통해, 빌드를 따로 만들지 않고도 팀원들에게 신속하게 프리뷰를 제공할
        수 있었습니다.
      </Paragraph>
      <Paragraph>
        <Tag>expo-updates</Tag> 패키지와 Expo Channel을 활용하여, CI/CD가 가능한
        빌드를 구축하였습니다. <Tag>eas.json</Tag>의{' '}
        <Tag>build.(프로파일명).channel</Tag>에 원하는 채널을 할당시키면, 그
        빌드는 프로파일에 해당되는 채널의 업데이트를 따라갑니다. (
        <ExternalAnchor href="https://docs.expo.dev/build/updates/">
          참고
        </ExternalAnchor>
        ) 이를 통해 코드만 푸시해도 업데이트가 되는 빌드를 구축함으로써, 무료
        플랜에서 월 최대 30회까지(Android 빌드 기준, iOS는 최대 15회)밖에 안
        되는 <Tag>eas build</Tag>의 사용 횟수를 획기적으로 줄일 수 있었습니다.
      </Paragraph>
    </main>
  );
}
