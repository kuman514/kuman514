...
export default function HanmogmPage() {
  return (
    ...
    >
      ...
      >
        <Paragraph>
          한모금 프로젝트는 사이드 프로젝트이자 소규모 팀 프로젝트인 만큼, 팀원
          한 명 한 명이 어떤 임팩트를 가하고 있는지 파악해야 한다는 점을
          배웠습니다. 예를 들어, 저는 프론트엔드 사이드의 전반적인 기능과
          유지보수 및 스토어 릴리즈 등등을 담당하므로, 제가 부재중일 경우 다른
          누군가가 전반적인 프론트엔드 사이드 작업들까지 맡아야 할 정도의
          임팩트를 가지고 있음을 인지하였습니다.
        </Paragraph>
        <Paragraph>
          저를 포함한 각 팀원들에겐 외부 일정이 큰 변수가 됩니다. 따라서, 야근
